...
import { findGridCoordinatesByAddress, findNearestGridFromLocations } from '../utils/coordinateConverter';

interface CampingSearchProps {
  onCampingSelected: (campingName: string, nx: number, ny: number, addr1: string) => void;
}
...
const CampingSearch: React.FC<CampingSearchProps> = ({ onCampingSelected }) => {
  ...
  const [selectedCamping, setSelectedCamping] = useState<CampingItem | null>(null);
  ...
  const handleSearch = async (text: string) => {
    ...
    try {
      ...
      if (err.response) {
        ...
        errorMessage = '서버로부터 응답이 없습니다. 인터넷 연결을 확인해주세요.';
      } else if (err.message) {
        // 요청 설정 중 오류가 발생한 경우
  ...
  const handleSelectCamping = (camping: any) => {
    try {
      ...
      if (camping.mapX && camping.mapY && !isNaN(camping.mapX) && !isNaN(camping.mapY)) {
        // locations.json에서 가장 가까운 좌표 찾기
        const coordinates = findNearestGridFromLocations(camping.mapX, camping.mapY);
        nx = coordinates.nx;
        ny = coordinates.ny;
        console.log(`locations.json 기반 좌표: nx=${nx}, ny=${ny}, 원본 좌표: mapX=${camping.mapX}, mapY=${camping.mapY}`);
      } else {
        // 경도/위도가 없는 경우 주소를 사용하여 좌표 추출
        const coordinates = findGridCoordinatesByAddress(camping.addr1);
        nx = coordinates.nx;
        ny = coordinates.ny;
        console.log(`주소 기반 좌표: nx=${nx}, ny=${ny}, 주소: ${camping.addr1}`);
      }
      ...
      Alert.alert('위치 정보 오류', '선택한 캠핑장의 위치 정보를 처리하는 중 오류가 발생했습니다.');
    }
  };

  return (
    <View style={styles.container}>
      <Text style={styles.title}>캠핑장 검색</Text>
      <Text style={styles.subtitle}>날씨를 확인할 캠핑장을 검색해주세요</Text>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            const regionName = addressParts.length >= 2 ? `${addressParts[0]} ${addressParts[1]}` : item.addr1;
            ...
            if (searchText && searchText.trim() !== '') {
              ...
              if (index !== -1) {
                ...
                const match = item.facltNm.substring(index, index + searchText.length);
                const after = item.facltNm.substring(index + searchText.length);
                
                campingNameDisplay = (
                  <Text>
                    {before}
                    <Text style={{fontWeight: 'bold', color: '#0066cc'}}>{match}</Text>
                    {after}
                  </Text>
                );
              }
              ...
              >
                <View style={styles.resultItemContent}>
                  <View style={styles.campingInfoContainer}>
                    ...
                    <Text style={styles.campingAddress} numberOfLines={1}>{item.addr1}</Text>
                  </View>
